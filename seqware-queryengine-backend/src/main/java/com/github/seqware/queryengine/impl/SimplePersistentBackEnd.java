...
public class SimplePersistentBackEnd implements BackEndInterface {
   ...
   *
   * @param fsi a {@link com.github.seqware.queryengine.impl.StorageInterface}
   * object.
   */
  ...
  public void update(Atom... objList) {
    ...
    if (Constants.TRACK_VERSIONING) {
      ...
      for (int i = 0; i < objList.length; i++) {
        Atom obj = objList[i];
        /* copy on write if tracking versions */
  ...
  @Override
  public void delete(Atom... objList) {
    storage.deleteAtom(objList);
  }
...
//    }
  ...
  /**
   * Given a collection of FeatureLists all with the same row key (and differing
   * timestamps), create a consistent view of the features in the feature set
   * corresponding effectively to the last timestamp
   *
