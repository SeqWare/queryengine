...
public class NaiveReadImporter extends Importer {
    ...
    public static final char READ_SET_PARAM = 'r';
    ...
    public static void main(String[] args) {
        SGID mainMethod = NaiveReadImporter.runMain(args);
        if (mainMethod == null) {
    ...
    public static SGID runMain(String[] args) {
        ...
        Option option4 = OptionBuilder.withArgName("readSet").withDescription("(optional) for benchmarking for now, append reads to an existing readset").hasArgs(1).create(READ_SET_PARAM);
        ...
        try {
            ...
            if (threads > 1){
                ...
                int linesInFile = NaiveReadImporter.countLines(iFile);
                int linesPerSegment = linesInFile/threads;
                // split up file
                LineIterator lineIterator = FileUtils.lineIterator(iFile);
                
                int lineCount = 0;
                File tmpFile = File.createTempFile("segment", "sam");
                while(lineIterator.hasNext()){
                    ...
                    if (lineIterator.hasNext()){
                        files.add(tmpFile);
                        tmpFile = File.createTempFile("segment", "sam");
                    }
                }
                files.add(tmpFile);
...
//            }
            ...
            // for benchmarking
            SGID readSetID = cmd.hasOption(READ_SET_PARAM) ? Utility.parseSGID(cmd.getOptionValue(READ_SET_PARAM)) : null;
            ...
            if (NaiveReadImporter.SECONDARY_INDEX_HACK){
                secondaryIndex = cmd.getOptionValue(SECONDARY_INDEX_PARAM);
            }
            
            SGID mainMethod = ReadImporter.performImport(threads, inputFiles, worker, compressed, outputFile, batch_size, readSetID, secondaryIndex);
            if (mainMethod == null) {
                return null;
            }
            return mainMethod;
            ...
            formatter.printHelp(NaiveReadImporter.class.getSimpleName(), options);
            Logger.getLogger(NaiveReadImporter.class.getName()).log(Level.SEVERE, null, ex);
